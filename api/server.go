...
// getTraderUserID 获取用于查询交易员的user_id（V1.44版本：恢复原始逻辑，直接使用admin）
func (s *Server) getTraderUserID(userID string) string {
	// V1.44版本：恢复原始逻辑，直接使用admin，不再转换为default
	// 这样创建和查询时使用相同的user_id，保持一致性
	return userID
}
...
func (s *Server) getTraderFromQuery(c *gin.Context) (*manager.TraderManager, string, error) {
	...
	traderUserID := s.getTraderUserID(userID)

	// 确保用户的交易员已加载到内存中
	err := s.traderManager.LoadUserTraders(s.database, traderUserID)
	if err != nil {
		log.Printf("⚠️ 加载用户 %s 的交易员失败: %v", traderUserID, err)
	}
	...
	if traderID == "" {
		...
		userTraders, err := s.database.GetTraders(traderUserID)
...
func (s *Server) handleCreateTrader(c *gin.Context) {
	userID := c.GetString("user_id")
	log.Printf("🚀 [创建交易员] 开始处理请求: userID=%s, 管理员模式=%v", userID, auth.IsAdminMode())
	
	var req CreateTraderRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		log.Printf("❌ [创建交易员] 请求参数解析失败: userID=%s, error=%v", userID, err)
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
	log.Printf("📋 [创建交易员] 请求参数: Name=%s, AIModelID=%s, ExchangeID=%s, InitialBalance=%.2f, BTCETHLeverage=%d, AltcoinLeverage=%d", 
		req.Name, req.AIModelID, req.ExchangeID, req.InitialBalance, req.BTCETHLeverage, req.AltcoinLeverage)

	// 校验杠杆值（允许0，会使用默认值）
	if req.BTCETHLeverage < 0 || req.BTCETHLeverage > 50 {
		log.Printf("❌ [创建交易员] 杠杆校验失败: BTCETHLeverage=%d (必须在0-50之间)", req.BTCETHLeverage)
		c.JSON(http.StatusBadRequest, gin.H{"error": "BTC/ETH杠杆必须在0-50倍之间（0表示使用默认值）"})
		return
	}
	if req.AltcoinLeverage < 0 || req.AltcoinLeverage > 30 {
		log.Printf("❌ [创建交易员] 杠杆校验失败: AltcoinLeverage=%d (必须在0-30之间)", req.AltcoinLeverage)
		c.JSON(http.StatusBadRequest, gin.H{"error": "山寨币杠杆必须在0-30倍之间（0表示使用默认值）"})
		return
	}
	log.Printf("✓ [创建交易员] 杠杆校验通过: BTC/ETH=%d, Altcoin=%d", req.BTCETHLeverage, req.AltcoinLeverage)
	...
	// 获取交易所配置（用于后续查询余额等操作）
	// V1.21版本：简化验证逻辑
	log.Printf("🔍 [创建交易员] 查找交易所配置: ExchangeID=%s, userID=%s", req.ExchangeID, userID)
	var exchangeCfg *config.ExchangeConfig
	exchanges, err := s.database.GetExchanges(userID)
	if err != nil {
		log.Printf("⚠️ [创建交易员] 获取用户交易所配置失败: userID=%s, error=%v（继续执行）", userID, err)
	} else {
		log.Printf("📊 [创建交易员] 用户交易所配置数量: %d", len(exchanges))
		for _, ex := range exchanges {
			if ex.ID == req.ExchangeID {
				exchangeCfg = ex
				log.Printf("✓ [创建交易员] 找到用户交易所配置: ID=%s, Enabled=%v", ex.ID, ex.Enabled)
				break
			}
		}
		// 如果当前用户没有配置，尝试从default用户获取
		if exchangeCfg == nil {
			log.Printf("🔍 [创建交易员] 用户未配置该交易所，尝试从default用户获取: ExchangeID=%s", req.ExchangeID)
			defaultExchanges, err := s.database.GetExchanges("default")
			if err != nil {
				log.Printf("⚠️ [创建交易员] 获取default用户交易所配置失败: error=%v", err)
			} else {
				log.Printf("📊 [创建交易员] default用户交易所配置数量: %d", len(defaultExchanges))
				for _, ex := range defaultExchanges {
					if ex.ID == req.ExchangeID {
						exchangeCfg = ex
						log.Printf("✓ [创建交易员] 找到default用户交易所配置: ID=%s, Enabled=%v", ex.ID, ex.Enabled)
						break
					}
				}
			}
		...
		if exchangeCfg == nil {
			log.Printf("⚠️ [创建交易员] 未找到交易所配置: ExchangeID=%s", req.ExchangeID)
		}
	}

	// 生成交易员ID
	traderID := fmt.Sprintf("%s_%s_%d", req.ExchangeID, req.AIModelID, time.Now().Unix())
	log.Printf("📝 [创建交易员] 生成交易员ID: %s", traderID)
	...
	if exchangeCfg == nil {
		...
		case "okx":
			tempTrader = trader.NewOKXTrader(exchangeCfg.APIKey, exchangeCfg.SecretKey, exchangeCfg.OKXPassphrase, exchangeCfg.Testnet)
		default:
	...
	// 创建交易员配置（数据库实体）
	// V1.44版本：恢复原始逻辑，直接使用admin作为user_id（不再转换为default）
	traderUserID := userID
	// 确保admin用户存在（如果使用admin作为user_id）
	if traderUserID == "admin" {
		if err := s.database.EnsureAdminUser(); err != nil {
			log.Printf("⚠️ [创建交易员] 确保admin用户存在失败: %v", err)
			// 不返回错误，继续执行
		} else {
			log.Printf("✓ [创建交易员] admin用户已确保存在")
		}
		log.Printf("ℹ️ [创建交易员] 管理员模式：使用admin作为user_id创建交易员 (userID=%s)", userID)
	} else {
		log.Printf("ℹ️ [创建交易员] 普通模式：使用原始user_id (userID=%s)", userID)
	}
	
	log.Printf("💰 [创建交易员] 余额信息: 用户输入=%.2f, 实际余额=%.2f", req.InitialBalance, actualBalance)
	log.Printf("⚙️ [创建交易员] 配置信息: BTCETHLeverage=%d, AltcoinLeverage=%d, ScanInterval=%d, IsCrossMargin=%v, SystemPromptTemplate=%s", 
		btcEthLeverage, altcoinLeverage, scanIntervalMinutes, isCrossMargin, systemPromptTemplate)
	
	trader := &config.TraderRecord{
		ID:                   traderID,
		UserID:               traderUserID, // 使用处理后的userID
		Name:                 req.Name,
	...
	log.Printf("📝 [创建交易员] 准备保存到数据库: ID=%s, UserID=%s, Name=%s, AIModelID=%s, ExchangeID=%s, InitialBalance=%.2f", 
		trader.ID, trader.UserID, trader.Name, trader.AIModelID, trader.ExchangeID, trader.InitialBalance)

	// 保存到数据库
	err = s.database.CreateTrader(trader)
	if err != nil {
		log.Printf("❌ [创建交易员] 数据库插入失败: error=%v", err)
		log.Printf("   [创建交易员] 交易员信息: ID=%s, Name=%s, UserID=%s, ExchangeID=%s, AIModelID=%s", 
			trader.ID, trader.Name, trader.UserID, trader.ExchangeID, trader.AIModelID)
		log.Printf("   [创建交易员] 杠杆配置: BTC/ETH=%d, Altcoin=%d", trader.BTCETHLeverage, trader.AltcoinLeverage)
		log.Printf("   [创建交易员] 管理员模式: %v, 原始userID=%s, traderUserID=%s", auth.IsAdminMode(), userID, traderUserID)
		log.Printf("   [创建交易员] 其他配置: IsCrossMargin=%v, ScanInterval=%d, UseCoinPool=%v, UseOITop=%v", 
			trader.IsCrossMargin, trader.ScanIntervalMinutes, trader.UseCoinPool, trader.UseOITop)
		
		c.JSON(http.StatusInternalServerError, gin.H{"error": fmt.Sprintf("创建交易员失败: %v", err)})
		return
	}

	log.Printf("✓ [创建交易员] 数据库插入成功: ID=%s, UserID=%s", trader.ID, trader.UserID)

	// 立即将新交易员加载到TraderManager中（使用traderUserID，因为交易员是用这个user_id创建的）
	log.Printf("🔄 [创建交易员] 开始加载交易员到内存: traderUserID=%s", traderUserID)
	err = s.traderManager.LoadUserTraders(s.database, traderUserID)
	if err != nil {
		log.Printf("⚠️ [创建交易员] 加载用户交易员到内存失败: traderUserID=%s, error=%v", traderUserID, err)
		// 这里不返回错误，因为交易员已经成功创建到数据库
	} else {
		log.Printf("✓ [创建交易员] 交易员已加载到内存: traderUserID=%s", traderUserID)
	}

	log.Printf("✅ [创建交易员] 创建成功: ID=%s, Name=%s, 模型=%s, 交易所=%s, UserID=%s", 
		traderID, req.Name, req.AIModelID, req.ExchangeID, traderUserID)
...
func (s *Server) handleUpdateTrader(c *gin.Context) {
	...
	// 检查交易员是否存在且属于当前用户
	traderUserID := s.getTraderUserID(userID)
	traders, err := s.database.GetTraders(traderUserID)
	if err != nil {
	...
	// 更新交易员配置（使用traderUserID，因为交易员是用这个user_id创建的）
	trader := &config.TraderRecord{
		ID:                   traderID,
		UserID:               traderUserID, // 使用traderUserID，保持与创建时一致
		Name:                 req.Name,
		AIModelID:            req.AIModelID,
		ExchangeID:           req.ExchangeID,
		...
		UseCoinPool:          existingTrader.UseCoinPool, // 保持原值
		UseOITop:             existingTrader.UseOITop, // 保持原值
	}
...
func (s *Server) handleDeleteTrader(c *gin.Context) {
	...
	// 从数据库删除（使用traderUserID，因为交易员是用这个user_id创建的）
	err := s.database.DeleteTrader(traderUserID, traderID)
	if err != nil {
...
func (s *Server) handleStartTrader(c *gin.Context) {
	...
	// 校验交易员是否属于当前用户（使用traderUserID，因为交易员是用这个user_id创建的）
	_, _, _, err := s.database.GetTraderConfig(traderUserID, traderID)
	if err != nil {
	...
	// 更新数据库中的运行状态（使用traderUserID，因为交易员是用这个user_id创建的）
	err = s.database.UpdateTraderStatus(traderUserID, traderID, true)
	if err != nil {
...
func (s *Server) handleStopTrader(c *gin.Context) {
	...
	// 更新数据库中的运行状态（使用traderUserID，因为交易员是用这个user_id创建的）
	err = s.database.UpdateTraderStatus(traderUserID, traderID, false)
	if err != nil {
		log.Printf("⚠️  更新交易员状态失败: %v", err)
	}
...
func (s *Server) handleSyncBalance(c *gin.Context) {
	...
	// 从数据库获取交易员配置（包含交易所信息）
	traderUserID := s.getTraderUserID(userID)
	traderConfig, _, exchangeCfg, err := s.database.GetTraderConfig(traderUserID, traderID)
	if err != nil {
		c.JSON(http.StatusNotFound, gin.H{"error": "交易员不存在"})
		return
	}
...
func (s *Server) handleGetTraderConfig(c *gin.Context) {
	...
	traderUserID := s.getTraderUserID(userID)
	traderConfig, _, _, err := s.database.GetTraderConfig(traderUserID, traderID)
	if err != nil {
...
// handleLatestDecisions 最新决策日志（返回所有记录，最新的在前）
func (s *Server) handleLatestDecisions(c *gin.Context) {
	...
	// V1.67版本：返回所有历史记录（最多10000条），而不是只返回5条
	records, err := trader.GetDecisionLogger().GetLatestRecords(10000)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"error": fmt.Sprintf("获取决策日志失败: %v", err),
...
func (s *Server) handleAdminLogin(c *gin.Context) {
	...
	// V1.44版本：确保admin用户存在后再生成token（恢复原始逻辑）
	// 确保admin用户存在（用于管理员模式下的交易员创建）
	log.Printf("🔧 [登录] 开始确保admin用户存在...")
	if err := s.database.EnsureAdminUser(); err != nil {
		log.Printf("⚠️ [登录] 确保admin用户存在失败: %v（继续执行）", err)
		// 不返回错误，因为可能已经存在
	} else {
		log.Printf("✓ [登录] admin用户已确保存在")
	}
...
// handleUpdatePromptTemplate 更新指定名称的提示词模板
func (s *Server) handleUpdatePromptTemplate(c *gin.Context) {
	templateName := c.Param("name")
	userID := c.GetString("user_id")
	
	log.Printf("📝 [更新提示词模板] 用户: %s, 模板: %s", userID, templateName)
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		log.Printf("❌ [更新提示词模板] 请求参数解析失败: %v", err)
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}

	// 更新模板
	err := decision.UpdatePromptTemplate(templateName, req.Content)
	if err != nil {
		log.Printf("❌ [更新提示词模板] 更新失败: %v", err)
		c.JSON(http.StatusInternalServerError, gin.H{"error": fmt.Sprintf("更新提示词模板失败: %v", err)})
		return
	}
	...
	if err != nil {
		log.Printf("⚠️ [更新提示词模板] 重新加载模板失败: %v", err)
		// 不返回错误，因为模板已经更新到文件
	}

	log.Printf("✓ [更新提示词模板] 模板已更新: %s", templateName)
	c.JSON(http.StatusOK, gin.H{
		"message": "提示词模板已更新",
		"name":    templateName,
	})
