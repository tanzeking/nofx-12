...
func (m *WSMonitor) initializeHistoricalData() error {
	...
	for _, symbol := range m.symbols {
		...
		go func(s string) {
			...
			// 获取历史K线数据（V1.65: 增加到300条以支持更准确的技术指标计算）
			klines, err := apiClient.GetKlines(s, "3m", DefaultKlineLimit)
			if err != nil {
			...
			// 获取历史K线数据（V1.65: 增加到300条以支持更准确的技术指标计算）
			klines4h, err := apiClient.GetKlines(s, "4h", DefaultKlineLimit)
			if err != nil {
				log.Printf("获取 %s 历史数据失败: %v", s, err)
				return
			}
...
func (m *WSMonitor) Start(coins []string) {
	...
	for attempt := 1; attempt <= maxRetries; attempt++ {
		...
		if attempt < maxRetries {
			...
			log.Printf("❌ 批量订阅流失败（已重试%d次）: %v", maxRetries, err)
			log.Printf("⚠️  将使用API方式获取市场数据（可能延迟）")
			return // 即使WebSocket失败，也允许继续运行（使用API作为后备）
		}
	}
	// 订阅所有交易对
	err = m.subscribeAll()
...
func (m *WSMonitor) GetCurrentKlines(symbol string, _time string) ([]Kline, error) {
	...
	if !exists {
		...
		apiClient := NewAPIClient()
		klines, err := apiClient.GetKlines(symbol, _time, DefaultKlineLimit)
		if err != nil {
