...
// UpdatePromptTemplate 更新指定名称的提示词模板（全局函数）
func UpdatePromptTemplate(name string, content string) error {
	return globalPromptManager.UpdateTemplate(name, content)
}

// UpdateTemplate 更新指定名称的提示词模板
func (pm *PromptManager) UpdateTemplate(name string, content string) error {
	pm.mu.Lock()
	defer pm.mu.Unlock()

	// 检查模板是否存在
	if _, exists := pm.templates[name]; !exists {
		return fmt.Errorf("提示词模板不存在: %s", name)
	}
	...
	if err != nil {
		return fmt.Errorf("保存提示词模板失败: %w", err)
	}
