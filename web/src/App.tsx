...
import { getRefreshIntervals } from './utils/refreshInterval'
import type {
  SystemStatus,
  AccountInfo,
  Position,
...
function App() {
  ...
  // V1.70版本：根据AI调用周期动态计算刷新间隔
  // 首先获取状态（包含scan_interval），然后根据它计算刷新间隔
  const { data: status } = useSWR<SystemStatus>(
    ...
    {
      refreshInterval: 15000, // 状态数据本身的刷新间隔（固定15秒，用于获取最新的scan_interval）
  ...
  const refreshIntervals = useMemo(() => {
    const intervals = getRefreshIntervals(status?.scan_interval)
    // V1.71版本：输出刷新间隔信息（用于调试）
    if (status?.scan_interval) {
      console.log(`[V1.71] AI扫描间隔: ${status.scan_interval}, 刷新间隔: 实时=${intervals.realtime}ms, 决策=${intervals.decision}ms, 统计=${intervals.statistics}ms`)
    }
    return intervals
  }, [status?.scan_interval])

  // 实时数据：账户、持仓、状态（使用动态刷新间隔）
  const { data: account } = useSWR<AccountInfo>(
    ...
    {
      refreshInterval: refreshIntervals.realtime, // 动态刷新间隔：AI周期 + 5秒
      revalidateOnFocus: false,
      dedupingInterval: Math.min(refreshIntervals.realtime / 2, 10000), // 去重间隔为刷新间隔的一半，最大10秒
    }
  )
  ...
  const { data: positions } = useSWR<Position[]>(
    ...
    () => api.getPositions(selectedTraderId),
    {
      refreshInterval: refreshIntervals.realtime, // 动态刷新间隔：AI周期 + 5秒
      revalidateOnFocus: false,
      dedupingInterval: Math.min(refreshIntervals.realtime / 2, 10000),
    }
  )
  ...
  const { data: decisions } = useSWR<DecisionRecord[]>(
    ...
    {
      refreshInterval: refreshIntervals.decision, // 动态刷新间隔：AI周期 + 5秒
      revalidateOnFocus: false,
      dedupingInterval: Math.min(refreshIntervals.decision / 2, 20000),
    }
  )
  ...
  const { data: stats } = useSWR<Statistics>(
    ...
    {
      refreshInterval: refreshIntervals.statistics, // 动态刷新间隔：AI周期 * 2
      revalidateOnFocus: false,
      dedupingInterval: Math.min(refreshIntervals.statistics / 2, 20000),
    }
  )
            ...
            <div>
              ...
              {decisions && decisions.length > 0 && (
                <div className="text-xs" style={{ color: '#848E9C' }}>
                  {language === 'zh' ? `所有历史周期 (${decisions.length}个)` : `All History Periods (${decisions.length})`}
                </div>
              )}
            </div>
          </div>
