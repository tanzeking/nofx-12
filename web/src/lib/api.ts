...
export const api = {
  ...
  // 获取提示词模板列表（无需认证）
  async getPromptTemplates(): Promise<{ templates: { name: string }[] }> {
    const res = await fetch(`${API_BASE}/prompt-templates`)
    if (!res.ok) throw new Error('获取提示词模板列表失败')
    return res.json()
  },

  // 获取指定提示词模板内容（无需认证）
  async getPromptTemplate(name: string): Promise<{
    name: string
    content: string
  }> {
    const res = await fetch(`${API_BASE}/prompt-templates/${name}`)
  ...
  async updatePromptTemplate(
    ...
    if (!res.ok) throw new Error('更新提示词模板失败')
    return res.json()
  },
}
