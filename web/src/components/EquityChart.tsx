...
export function EquityChart({ traderId }: EquityChartProps) {
  ...
  const refreshIntervals = useMemo(() => {
    return getRefreshIntervals(status?.scan_interval)
  ...
  const { data: history, error } = useSWR<EquityPoint[]>(
    ...
    {
      refreshInterval: refreshIntervals.history, // 动态刷新间隔：AI周期 * 2
      revalidateOnFocus: false,
      dedupingInterval: Math.min(refreshIntervals.history / 2, 20000),
    }
  )
