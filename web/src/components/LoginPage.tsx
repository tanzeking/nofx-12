...
export function LoginPage() {
  ...
  const [adminMode, setAdminMode] = useState<boolean | null>(true) // 默认管理员模式，因为后端默认是管理员模式
  ...
  useEffect(() => {
    getSystemConfig()
      ...
      .catch(() => {
        // 获取配置失败时，默认使用管理员模式（因为后端默认是管理员模式）
        setAdminMode(true)
        setConfigLoading(false)
      })
  }, [])
      ...
      >
        <div className="w-full max-w-md">
          ...
          <div className="text-center mb-8">
            ...
            >
              {configLoading ? '正在加载...' : adminMode ? '请输入管理员密码' : step === 'login' ? '请输入您的邮箱和密码' : '请输入两步验证码'}
            </p>
          </div>
