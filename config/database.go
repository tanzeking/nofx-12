...
func (d *Database) createTables() error {
	queries := []string{
		// AI模型配置表（移除外键约束定义，与GitHub最新版本保持一致）
		...
		// 交易所配置表（移除外键约束定义，与GitHub最新版本保持一致）
		...
		// 用户信号源配置表（移除外键约束定义，与GitHub最新版本保持一致）
		...
		// 交易员配置表（移除外键约束定义，与GitHub最新版本保持一致）
...
func (d *Database) initDefaultData() error {
	// 首先创建 "default" 用户（如果不存在），因为 ai_models 和 exchanges 表有外键约束
	// 注意：default 用户是一个虚拟用户，用于存储系统默认配置
	_, err := d.db.Exec(`
	...
	aiModels := []struct {
		...
		{"qwen", "Qwen", "qwen"},
		{"googleai", "Google AI", "googleai"},
		{"chatgpt", "ChatGPT", "chatgpt"},
		{"gpts", "GPTs (Assistant API)", "gpts"},
	}
...
func (d *Database) EnsureAdminUser() error {
	...
	if err != nil {
		log.Printf("❌ [数据库] 检查admin用户是否存在失败: error=%v", err)
		return err
	}

	// 如果已存在，直接返回
	if count > 0 {
		log.Printf("✓ [数据库] admin用户已存在")
		return nil
	}

	// 创建admin用户（密码为空，因为管理员模式下不需要密码）
	log.Printf("🔧 [数据库] admin用户不存在，开始创建...")
	adminUser := &User{
		ID:           "admin",
	...
	err = d.CreateUser(adminUser)
	if err != nil {
		log.Printf("❌ [数据库] 创建admin用户失败: error=%v", err)
		return err
	}
	log.Printf("✓ [数据库] admin用户创建成功")
	return nil
}
...
func (d *Database) UpdateAIModel(userID, id string, enabled bool, apiKey, customAPIURL, customModelName string) error {
	...
	if provider == id && (provider == "deepseek" || provider == "qwen" || provider == "googleai" || provider == "chatgpt") {
		// id 本身就是 provider
		provider = id
	} else {
	...
	if err != nil {
		...
		if provider == "deepseek" {
			...
			name = "ChatGPT"
		} else if provider == "gpts" {
			name = "GPTs (Assistant API)"
		} else {
			name = provider + " AI"
		}
	}
...
// CheckAIModelExists 检查AI模型ID是否存在（不检查user_id，因为外键约束只检查id）
func (d *Database) CheckAIModelExists(modelID string) (bool, error) {
	var count int
	err := d.db.QueryRow(`SELECT COUNT(*) FROM ai_models WHERE id = ?`, modelID).Scan(&count)
	if err != nil {
		return false, err
	}
	return count > 0, nil
...
// CheckExchangeExists 检查交易所ID是否存在（不检查user_id，因为外键约束只检查id）
func (d *Database) CheckExchangeExists(exchangeID string) (bool, error) {
	var count int
	err := d.db.QueryRow(`SELECT COUNT(*) FROM exchanges WHERE id = ?`, exchangeID).Scan(&count)
	if err != nil {
		return false, err
	}
	return count > 0, nil
...
// CreateTrader 创建交易员（V1.44版本：恢复原始逻辑，支持admin和default用户）
func (d *Database) CreateTrader(trader *TraderRecord) error {
	log.Printf("🔧 [数据库] CreateTrader 开始: ID=%s, UserID=%s, Name=%s", trader.ID, trader.UserID, trader.Name)
	
	// V1.44版本：确保用户存在（支持admin和default）
	if trader.UserID == "admin" {
		log.Printf("🔧 [数据库] 检查admin用户是否存在")
		if err := d.EnsureAdminUser(); err != nil {
			log.Printf("❌ [数据库] 确保admin用户存在失败: error=%v", err)
		} else {
			log.Printf("✓ [数据库] admin用户已确保存在")
		}
	} else if trader.UserID == "default" {
		log.Printf("🔧 [数据库] 检查default用户是否存在")
	_, err := d.db.Exec(`
			INSERT OR IGNORE INTO users (id, email, password_hash, otp_verified, created_at, updated_at)
			VALUES ('default', 'default@system.local', '', 0, datetime('now'), datetime('now'))
		`)
		if err != nil {
			log.Printf("❌ [数据库] 确保default用户存在失败: error=%v", err)
		} else {
			log.Printf("✓ [数据库] default用户已确保存在")
		}
	}
	
	// 检查AI模型是否存在
	log.Printf("🔍 [数据库] 检查AI模型是否存在: AIModelID=%s", trader.AIModelID)
	var modelCount int
	err := d.db.QueryRow(`SELECT COUNT(*) FROM ai_models WHERE id = ?`, trader.AIModelID).Scan(&modelCount)
	if err != nil {
		log.Printf("⚠️ [数据库] 检查AI模型失败: AIModelID=%s, error=%v", trader.AIModelID, err)
	} else if modelCount == 0 {
		log.Printf("⚠️ [数据库] AI模型不存在: AIModelID=%s (可能存在外键约束问题)", trader.AIModelID)
	} else {
		log.Printf("✓ [数据库] AI模型存在: AIModelID=%s", trader.AIModelID)
	}
	
	// 检查交易所是否存在
	log.Printf("🔍 [数据库] 检查交易所是否存在: ExchangeID=%s", trader.ExchangeID)
	var exchangeCount int
	err = d.db.QueryRow(`SELECT COUNT(*) FROM exchanges WHERE id = ?`, trader.ExchangeID).Scan(&exchangeCount)
	if err != nil {
		log.Printf("⚠️ [数据库] 检查交易所失败: ExchangeID=%s, error=%v", trader.ExchangeID, err)
	} else if exchangeCount == 0 {
		log.Printf("⚠️ [数据库] 交易所不存在: ExchangeID=%s (可能存在外键约束问题)", trader.ExchangeID)
	} else {
		log.Printf("✓ [数据库] 交易所存在: ExchangeID=%s", trader.ExchangeID)
	}
	
	log.Printf("📝 [数据库] 准备执行INSERT语句: ID=%s, UserID=%s, Name=%s, AIModelID=%s, ExchangeID=%s, InitialBalance=%.2f", 
	...
	if err != nil {
		log.Printf("❌ [数据库] INSERT失败: ID=%s, UserID=%s, error=%v", trader.ID, trader.UserID, err)
		log.Printf("   [数据库] 错误详情: %+v", err)
		// 尝试获取更详细的错误信息
		if sqlErr, ok := err.(interface{ Error() string }); ok {
			log.Printf("   [数据库] SQL错误信息: %s", sqlErr.Error())
		}
	} else {
		log.Printf("✓ [数据库] INSERT成功: ID=%s, UserID=%s", trader.ID, trader.UserID)
	}
