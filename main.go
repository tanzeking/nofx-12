...
func main() {
	...
	// 初始化数据库配置
	// V1.75版本：支持环境变量配置存储路径（用于 Hugging Face 等云平台部署）
	...
	if hfDataPath := os.Getenv("HF_HOME"); hfDataPath != "" {
		// 如果存在 HF_HOME，使用 /data 目录（Hugging Face 持久化存储）
		if _, err := os.Stat("/data"); err == nil {
			dbPath = "/data/config.db"
			log.Printf("📦 检测到 Hugging Face 环境，使用持久化存储: %s", dbPath)
		}
	}
	...
	// V1.76版本：Hugging Face Spaces 使用固定端口 7860
	// 优先使用环境变量 PORT（Hugging Face 会自动设置）
	if hfPort := os.Getenv("PORT"); hfPort != "" {
		apiPortStr = hfPort
		log.Printf("📦 检测到 Hugging Face 环境，使用端口: %s", apiPortStr)
	} else if apiPortStr == "" {
		apiPortStr = "8080" // 默认端口
	}
